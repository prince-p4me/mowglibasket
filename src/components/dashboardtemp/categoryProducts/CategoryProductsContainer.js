...
const CategoryProductsContainer = (props) => {
    ...
    const { item, navigateToProductList,
        navigateToProductDetails, response, hideViewAll } = props

    const RenderItem = ({ item, index }) => <ProductItemContainer
        item={item}
        response={response}
    ...
    return (
        <View>
            <View style={{
                ...
                {
                    (hideViewAll && hideViewAll == true) ? null :
